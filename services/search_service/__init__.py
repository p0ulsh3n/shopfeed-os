...
Handles:
    - Elasticsearch full-text search with boosting
    - Context-aware search (shows live streams if "live" is in query)
    - Autocomplete with Typesense
    - Search result ranking (uses commerce score)
"""
...
async def search(
    ...
    Features:
        - Multi-field search (title, description, tags, brand)
        - Live context: if query contains "live", boost live streams
        - Category filtering
    ...
    # Simple text matching (Elasticsearch in production)
    results: list[SearchResult] = []
    ...
    for product in _search_index:
        ...
        # Simple relevance: count query terms in fields
